[Previous](create-tables-using-vector-data-type.html) [Next](load-vector-data-using-sqlloader.html) JavaScript must be enabled to correctly display this
content
...
Here's an additional example demonstrating the insertion of `SPARSE` vectors
into a table:
