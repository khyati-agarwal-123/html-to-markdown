[Previous](jaccard-similarity.html) [Next](l1_distance.html) JavaScript must
...
follows](https://docs.oracle.com/en/database/oracle/oracle-database/23/vecse/img/vector_distance.gif)[Description of the illustration
vector_distance.eps](https://docs.oracle.com/en/database/oracle/oracle-
database/23/vecse/img_text/vector_distance.html)
...
metric, then the default distance metric is cosine. If the input vectors are
`BINARY` vectors, the default metric is hamming .
...
On the other hand, if the indexed columns do not have a common metric, or none
of the columns have an index defined, then the metric `COSINE` is used.
...
If you use `JACCARD_DISTANCE` or the `JACCARD` metric, then `expr1` and
`expr2` must evaluate to `BINARY` vectors.
...
[← Previous](jaccard-similarity.md)
