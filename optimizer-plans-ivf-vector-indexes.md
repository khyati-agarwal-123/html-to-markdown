[Previous](optimizer-plans-hnsw-vector-indexes.html) [Next](vector-index-hints.html) JavaScript must be enabled to correctly display this content
...
When using IVF vector indexes, you can see two possible plans for your
similarity searches:
...
IVF indexes use auxiliary partition tables and associated indexes to store
index information. Mainly `VECTOR$<index name>$<ids>$IVF_FLAT_CENTROIDS` and
`VECTOR$<index name>$<ids>$IVF_FLAT_CENTROIDS_PARTITIONS`.
