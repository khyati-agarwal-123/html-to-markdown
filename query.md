[Previous](load_onnx_model_cloud.html) [Next](rebuild_index.html) JavaScript
...
Query is overloaded and supports a version with `query_vector` passed in as a
`VECTOR` type in addition to `CLOB`.
...
This function accepts the input data type as `VARCHAR2`, `NUMBER`, `JSON`,
`BOOLEAN` or `CLOB`.
