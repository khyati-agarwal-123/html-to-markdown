[Previous](import-pretrained-models-onnx-format-vector-generation-database.html) [Next](convert-pretrained-models-onnx-model-end-end-instructions.html) JavaScript must be enabled to correctly display this content 
...
**Parent topic:** [Import Pretrained Models in ONNX Format for Vector Generation Within the Database](import-pretrained-models-onnx-format-vector-generation-database.html "You can download pretrained embedding machine learning models, convert them into ONNX format if they are not already in ONNX format, import the ONNX format models into Oracle Database, and generate vector embeddings from your data within the database.")
