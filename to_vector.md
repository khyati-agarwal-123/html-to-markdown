[Previous](vector-constructors.html) [Next](vector1.html) JavaScript must be
...
`TO_VECTOR` is a constructor that takes a string of type `VARCHAR2`, `CLOB`,
`BLOB`, or `JSON` as input, converts it to a vector, and returns a vector as
output. `TO_VECTOR` also takes another vector as input, adjusts its format,
and returns the adjusted vector as output. `TO_VECTOR` is synonymous with
`VECTOR`.
...
![Description of to_vector_vecse.eps
follows](https://docs.oracle.com/en/database/oracle/oracle-database/23/vecse/img/to_vector_vecse.gif)[Description of the illustration
to_vector_vecse.eps](https://docs.oracle.com/en/database/oracle/oracle-
database/23/vecse/img_text/to_vector_vecse.html)
...
[← Previous](vector-constructors.md)
