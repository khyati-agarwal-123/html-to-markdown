...
generation models by Cohere, Generative AI, Google AI, Hugging Face, OpenAI,
or Vertex AI. The input is a textual prompt, and the generated output is a
...
To generate a text response for the prompt "`What is Oracle Text?`", using an
external LLM:
...
Here, the cohere.command-r-16k model is used. You can replace `model` with
your own value, as required.
...
If you want to pass any additional REST provider-specific parameters, then you
must enclose those in `chatRequest`.
